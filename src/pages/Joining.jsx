...
const handleJoiningSubmit = async (e) => {
  ...
  try {
    ...
    // FIX: Proper timestamp format for Column X (23) - dd/mm/yyyy format
    const now = new Date();
    ...
    // Existing columns A to AA (columns 0 to 26)
    rowData[0] = `${month}/${day}/${year} ${now.getHours()}:${now.getMinutes()}:${now.getSeconds()}`;           // Column A: Timestamp with time
    rowData[1] = serialNumber;                    // Column B: Joining ID (AUTO GENERATED)
    rowData[2] = selectedItem.candidateName;   // Column C: Name As Per Aadhar
    rowData[3] = joiningFormData.fatherName;   // Column D: Father Name
    rowData[4] = formatDateForStorage(joiningFormData.dateOfJoining); // Column E: Date Of Joining (mm/dd/yyyy)
    ...
    // FIX: Column X (index 23) - Date only in dd/mm/yyyy format 
    rowData[23] = ''; // Column X: Date in dd/mm/yyyy format
